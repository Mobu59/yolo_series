...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self,
            ...
            for p in path if isinstance(path, list) else [path]:
                ...
                if p.is_dir():  # dir
                    ...
                    if p.name.endswith(".json"):
                        img_list, label_list = _parse_dataset(p)
                        f += img_list
                    else:    
                        with open(p) as t:
        ...
        # Check cache
        if self.path.endswith(".json"):
            self.label_files = label_list  # labels
            cache_path = Path(Path(path).parent).with_suffix('.cache')
        else:    
...
def verify_image_label(args):
    ...
    im_file, lb_file, prefix = args #single_img_path，single_label，train:
    ...
    try:
        ...
        # verify labels
        #for json dataset
        if isinstance(lb_file, list):
            nf = 1  # label found
            lb = _parse_line(lb_file, shape)
            nl = len(lb)
            if nl:
                assert lb.shape[1] == 5, f'labels require 5 columns, {lb.shape[1]} columns detected'
                assert (lb >= 0).all(), f'negative label values {lb[lb < 0]}'
                #assert (lb[:, 1:] <= 1).all(), f'non-normalized or out of bounds coordinates {lb[:, 1:][lb[:, 1:] > 1]}'
                assert (lb[:, 1:] <= 1).all(), f'non-normalized or out of bounds coordinates {lb} {im_file}'
...
def _parse_line(line, shape):
    ...
    for i in line:
        ...
        x1 = i['xmax']
        y1 = i['ymax']
        cx = (x0 + x1) / 2.0 / shape[0]
        cy = (y0 + y1) / 2.0 / shape[1]
        w = (x1 - x0) / shape[0]
        h = (y1 - y0) / shape[1]
        lb.append([name, cx, cy, w, h])
